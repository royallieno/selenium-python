from selenium import webdriver
from selenium.webdriver.support.select import Select

driver = webdriver.Chrome(executable_path="C:\\chromedriver.exe")
driver.get("https://rahulshettyacademy.com/angularpractice/")

# driver.find_element_by_name("name").send_keys("Arshad")
driver.find_element_by_css_selector("input[name='name']").send_keys("Gulnawaz")
driver.find_element_by_name("email").send_keys("Arshad")

driver.find_element_by_id("exampleCheck1").click()

# select class provide the methods to handle the option in dropdown.
dropdown = Select(driver.find_element_by_id("exampleFormControlSelect1"))
dropdown.select_by_visible_text("Female")
dropdown.select_by_index(0)
# dropdown.select_by_value("M")

driver.find_element_by_xpath("//input[@type='submit']").click()

# successText = driver.find_element_by_class_name("alert-success").text
# print(successText)

message = driver.find_element_by_class_name("alert-success").text
assert "success" in message

# //*[contains(@class,'alert-success')] - Xpath - RegularExpression
# [class*='alert-success'] - CSS - RegularExpression
